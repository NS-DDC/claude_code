...
/**
 * 파일 시스템 직접 순회 — 삭제 흔적 디렉토리 스캔
 *
 * [스캔 대상]
 * - .Trash / .Trashes (삼성 One UI 휴지통)
 * - .recently-deleted (일부 OEM 휴지통)
 * - .thumbnails (삭제된 사진의 썸네일 잔존)
 * - lost+found (파일 시스템 복구 잔여)
 * - .recycle / .FileManagerTrash (파일 매니저 휴지통)
 *
 * [안전 장치]
 * - 최대 깊이 5
 * - 심볼릭 링크 순환 감지
...
class FileSystemDataSource {
    ...
    companion object {
        ...
        private val RECOVERY_SCAN_DIRS = listOf(
            ...
            "/storage/emulated/0/.Recycle",
            // 삼성 보안 폴더 외부 삭제 임시
            "/storage/emulated/0/.SamsungTrash",
        )
    }
    ...
    private suspend fun scanDirectory(
        ...
        for (child in children) {
            ...
            if (child.isDirectory) {
                ...
                // categoryOf는 이제 항상 FileCategory를 반환 (null 없음)
                val category = FileExtensions.categoryOf(ext)

                // OTHER 카테고리는 1KB 미만 제외 (시스템 잔여 파일 필터)
                if (category == FileCategory.OTHER && child.length() < 1024L) continue
